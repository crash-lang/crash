...
pub struct LexingRule{
    patterns: Vec<Regex>,
    typ: TokenType<'static>
}

impl LexingRule {

    pub fn new(patterns: Vec<Regex>, typ: TokenType<'static>) -> Box<Self> {
        Box::new(Self { patterns, typ })
    }
    ...
    pub fn add_delimiter(&mut self, open: String, escape: String, closing: String) {
        let starting_symbol = regex_escape(&open);
        let escape_symbol = regex_escape(&escape);
        let closing_symbol = regex_escape(&closing);
    ...
    pub fn typ(&self) -> &TokenType<'static> {
        &self.typ
    }


    pub(crate) fn patterns(&self) -> &Vec<Regex> {
        &self.patterns
    }
}
